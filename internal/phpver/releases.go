...
// Release is a downloadable NTS x64 Windows build of PHP.
type Release struct {
	Version string
	ZipURL  string
}
...
// the current branches first, then from the archives for EOL versions.
func Resolve(ctx context.Context, sel Selector) (Release, error) {
	...
	return Release{}, fmt.Errorf("no Windows x64 build found for PHP %s", sel)
}
